...
function buildServer(lang) {
  const server = new McpServer({ name: "outreach-agent", version: "1.0.0" });
...
// Endpoint dedicato agente/MCP: autenticazione agente (Bearer agtok_),
// transport Streamable HTTP stateless, nessuna sessione.
router.post("/api/mcp", requireAgent, async (req, res) => {
  try {
    const server = buildServer(req.headers["accept-language"] || "en");
